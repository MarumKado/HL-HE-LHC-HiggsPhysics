...
We implemented the ATLAS and CMS HL-LHC projections for Higgs signal strength measurements for the individual production times decay modes (see Section~\ref{sec:2:exp_combination}) into the code \texttt{Higgs\-Signals}~\cite{Bechtle:2013xfa,Bechtle:2014ewa}, including the corresponding correlation matrices.
% assuming YR18 systematic uncertainties (S2).
We consider the projections for both future scenarios S1 (with Run~2 systematic uncertainties) and S2 (with YR18 systematic uncertainties)~\cite{HLHELHCCommonSystematics}, see Sec.~\ref{sec:expcomb_prodtimesdecay}, Tab.~\ref{tab:summary_A1_5PD} {(\color{green} make sure table reference is correct once S2 results are included!)}.
%These scenarios differ in the treatment of systematic uncertainties. In S1, systematic uncertainties are unchanged {with respect to current values}; S2 employs rescaled experimental systematic uncertainties~\cite{HLHELHCCommonSystematics} and furthermore assumes a reduction of theoretical uncertainties by a factor 2. 
