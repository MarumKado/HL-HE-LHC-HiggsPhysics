...
The expected performance for the $b\bar{b}\gamma\gamma$ channel is assessed through the use of truth-level Monte Carlo (MC) samples. 
...
The systematic uncertainties follow the common recommendations for HL-LHC studies~\cite{ATLASperfPUBnote}. 
...
\begin{figure}[!htb]
\centering 
\subfloat[]{\label{fig:ATLAS_HH_4b_a}\includegraphics[width=0.5\textwidth]{\main/section3/plots/ATLAS_LimitVsSyst_Preliminary.eps}} 
...
The final analysis discriminant, $\mhh$, showed in Figure~\ref{fig:ATLAS_HH_4b_b} is the invariant mass of the selected four-jet system, after a correction based on the known Higgs boson mass. The significance neglecting the systematic uncertainties is 1.39 standard deviation, while it is 0.61 standard deviation when the current systematic uncertainties are included. If the systematic uncertainty on the multijet background were scaled by the luminosity the significance would be XX standard deviations.
...
\begin{figure}[!htb]
\centering 
\subfloat[$\tau_{lep}\tau_{had}$ channel passing a single-lepton trigger (SLT)]{\label{fig:ATLAS_HH_bbtautau_a}\includegraphics[width=0.5\textwidth]{\main/section3/plots/HIGG-2016-16_fig_01a.pdf}} 
\subfloat[$\tau_{lep}\tau_{had}$ channel passing a lepton-plus-$\tau_{had}$ trigger (LTT)]{\label{fig:ATLAS_HH_bbtautau_b}\includegraphics[width=0.5\textwidth]{\main/section3/plots/HIGG-2016-16_fig_01c.pdf}}\\
\subfloat[$\tau_{had}\tau_{had}$ channel]{\label{fig:ATLAS_HH_bbtautau_c}\includegraphics[width=0.5\textwidth]{\main/section3/plots/HIGG-2016-16_fig_01e.pdf}}
\subfloat[]{\label{fig:ATLAS_HH_bbtautau_d}\includegraphics[width=0.5\textwidth]{\main/section3/plots/ATLAS_bbtautau_LHscan.pdf}}
...
\begin{figure}[!htb]
\centering 
\subfloat[]{\label{fig:ATLAS_HH_bbyy_a}\includegraphics[width=0.5\textwidth]{\main/section3/plots/bbyy_myy_SignalBackground_2GeVbins_Preliminary.pdf}} 
\subfloat[]{\label{fig:ATLAS_HH_bbyy_b}\includegraphics[width=0.5\textwidth]{\main/section3/plots/mbbyy_SignalBackground_20GeVbins_variouskappa_Preliminary.pdf}} 
...
The 68\% Confidence Intervals for $\kappa_{\lambda}$, from the likelihood ratio test performed on the Asimov dataset created from the
backgrounds and the SM HH signal are $0.4 \leq \kl\ \leq 1.7$ and $0.25 \leq \kl\ \leq 1.9$ with and without systematic uncertainties respectively. The Confidence Intervals per channel are summarised in Table~\ref{tab:ATLAS_HH_kappalambda}.
...
$HH \rightarrow b\bar{b}\tau\tau$ &  $0.2 \leq \kappa_{\lambda} \leq 2.0 \cup 5.9 \leq \kappa_{\lambda} \leq 7.2$ & $0.0 \leq \kappa_{\lambda} \leq 2.3 \cup 5.7 \leq \kappa_{\lambda} \leq 7.8$ \\
$HH \rightarrow b\bar{b}\gamma\gamma$ &  $0.0 \leq \kappa_{\lambda} \leq 2.1$ & $0.0 \leq \kappa_{\lambda} \leq 2.1$ \\
...
%Considering only statistical uncertainties the trilinear coupling modifier is measured to be:
%%\begin{equation}
%\[
%\begin{split}
...
%%\end{equation}
%\]
%Considering statistical and systematic uncertainties the trilinear coupling modifier is measured to be:
%%\begin{equation}
%\[
%\begin{split}
%1^{+XX}_{-XX}  \text{ at 68\% CL} \\
