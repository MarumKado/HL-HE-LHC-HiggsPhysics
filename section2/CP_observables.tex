...
\begin{table*}
        \begin{center}
                ...
                \end{tabular}
                \caption{Estimated uncertainties [\%] on the determination of single-Higgs production channels in $H \to 4\ell$ decay mode. These are CMS projections for high-luminosity LHC (14 TeV center of mass energy and 3 ab$^{-1}$ integrated luminosity) in scenario S1 (systematic uncertanties are kept constant with luminosity) taken from CMS-PAS-FTR-18-011.}\label{tab:one}
        \end{center}
\end{table*}
...
Since we are interested in the sensitivity on the CPV parameters that can be reached at HL and HE LHC and for which we don't have any experimental data yet, we take 
$\mu_{i,f}^{\rm obs.}=1$, implying that the future data would be consistent with the SM hypothesis. In the current analysis, we consider all the single Higgs production channels and Higgs decaying to four charged-leptons, {\it i.e} $i={{\rm ggF, VBF}, ZH, WH, t{\bar t}H}$ and $f=4\ell (2e2\mu, 4e, 4\mu)$. The projected uncertainties in these channels for HL-LHC are given in table~\ref{tab:one}. Note that only the $H\to 4\ell $ decay mode has a non-trivial kinematic distribution and therefore other decay modes in the present analysis have been ignored.
...
is one of the most sensitive kinematic observables to the CP-Violating $VVH$ couplings~\cite{}. We have considered 50 bins of $\phi$-distribution to perform the fit at differential level. For each bin, we calculate the signal strength ($\mu_{4\ell,j}; j=1\to50$) corresponding 
